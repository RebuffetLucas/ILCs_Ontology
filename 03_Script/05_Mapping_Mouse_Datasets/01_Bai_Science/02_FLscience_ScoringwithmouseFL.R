...
if (DO_SAVE_FIGURE == TRUE){
  png(file=paste0( SAVE_FIG_PATH ,"DotPlot_TOP_",FINDMARKERS_SHOWTOP ,".png") , width = 30, height = 25,  units = "cm", res=600 )
  print(p10)
  dev.off()
  
  pdf(file=paste0( SAVE_FIG_PATH ,"DotPlot_TOP_",FINDMARKERS_SHOWTOP ,".pdf") , width = 30/2.54, height = 25/2.54 )
  print(p10)
  dev.off()
...
if (DO_SAVE_FIGURE == TRUE){
  ...
  for (names_signatures in names(gene_list_by_cluster)){
    ...
    png(file=paste0( SAVE_FIG_PATH,"/VlnPlot_",names_signatures,".png") , width = 45, height = 25,  units = "cm", res=600 )
    print(p2)
    dev.off()
  
    pdf(file=paste0( SAVE_FIG_PATH,"/VlnPlot_",names_signatures,".pdf") , width = 25/2.54, height = 15/2.54)
    print(p2)
    dev.off()
    
    
    png(file=paste0( SAVE_FIG_PATH ,"/FeaturePlot_Score_",names_signatures,".png") , width = 15, height = 12.5,  units = "cm", res=600  )
    print(p1)
    dev.off()
    
    
    pdf(file=paste0( SAVE_FIG_PATH ,"/FeaturePlot_Score_",names_signatures,".pdf") , width = 15/2.54, height = 12.5/2.54  )
    print(p1)
    dev.off()
