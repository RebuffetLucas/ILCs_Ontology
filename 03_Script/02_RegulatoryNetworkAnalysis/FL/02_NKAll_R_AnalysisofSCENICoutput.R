### Analysis of SCENIC Output for FL####
...
rows_with_nan <- apply(regulonActivity_byCellType_Scaled, 1, function(row) any(is.nan(row)))

# Extract row names that have NaN values
rownames_with_nan <- rownames(regulonActivity_byCellType_Scaled)[rows_with_nan]

# Print the row names with NaN
print(rownames_with_nan)
...
plotRSS_oneSet(rss, setName = "C1_ILCP", n=number_regulons_RSS_showtop) # cluster ID
plotRSS_oneSet(rss, setName = "C2_Klrd1", n=number_regulons_RSS_showtop) # cluster ID
plotRSS_oneSet(rss, setName = "C3_ILC3P", n=number_regulons_RSS_showtop) # cluster ID
plotRSS_oneSet(rss, setName = "C4_EILP", n=number_regulons_RSS_showtop) # cluster ID
plotRSS_oneSet(rss, setName = "C5_Apoe", n=number_regulons_RSS_showtop) # cluster ID
...
pheatmap::pheatmap(topregulonsplot, fontsize_row=8, fontsize_col = 20,
...
#Plot for dvt of NK cells / Need a series of regulons of interest here
...
#### Vizualisation with tsne etc... => Has to be done, necessit embedings ! 
...
#Potential regulator WITH MULTIPLE GENES
