## @knitr SubClustand_Scoring_With_Mouse_FL_Signatures_AfterHarmony

PBMC3 = subset(PBMC2, idents= SUBSET_TO_SUBCLUSTER)

cat(" \n \n")
cat("## Subclustering Human FL " , SUBSET_TO_SUBCLUSTER , " HARMONIZATION and scoring with with mouse FL signatures")
cat(" \n \n")
...
PBMC3 = RunHarmony(PBMC3, HARMONIZATION_CRITERIA)


PBMC3 <- FindNeighbors(PBMC3, reduction= "harmony", dims = 1:20)
PBMC3 <- FindClusters(PBMC3, resolution = RESOLUTION_SUBCLUST)

PBMC3 <- RunUMAP(PBMC3, reduction = "harmony",dims = 1:20)
